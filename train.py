...
if __name__ == '__main__':
    ...
    if args.backbone == 'segman':
        FSSDataset.initialize(img_size=224, datapath=args.datapath, use_original_imgsize=False)
    else:
    ...
    dataloader_val = FSSDataset.build_dataloader(args.benchmark, args.bsz, args.nworker, args.fold, 'val')
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.nepoch)
    ...
    for epoch in range(args.nepoch):
        ...
        trn_loss, trn_miou, trn_fb_iou = train(epoch, model, dataloader_trn, optimizer, training=True, scaler=scaler)
        # evaluation
        # if args.local_rank == 0:
        with torch.no_grad():
