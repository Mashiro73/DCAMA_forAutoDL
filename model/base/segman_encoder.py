...
try:
    # ✅ [修改] 直接从当前目录 (.) 导入 csm_triton
    from .csm_triton import CrossScanTriton, CrossMergeTriton
except ImportError as e:
    # 可以保留 try-except 块以增加健壮性，或者直接移除
    print(f"Error importing csm_triton: {e}")
    # 如果确定文件就在同级，可以直接写成:
    # from .csm_triton import CrossScanTriton, CrossMergeTriton
    raise e # 明确抛出错误，方便调试
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # ✅ [修改点 1A] 新增一个空列表，用于收集这个 Stage 内所有 Block 的输出
        feats_from_blocks = []
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # ✅ [修改点 1B] 将每个 Block 的输出特征图添加到列表中
            # 注意：这里的特征图是 [B, C, H, W] 格式，无需转换，可以直接使用
            feats_from_blocks.append(x.clone().detach())

        # ✅ [修改点 1C] 将 forward 的返回值从 x 更改为 (feats_from_blocks, x)
        return feats_from_blocks, x
...
class SegMANEncoder(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # ✅ [修改点 2A] 新增一个实例属性 self.feat_maps
        # 这将作为暴露给外部 (DCAMA.py) 的公共接口，与 SwinV2 改造完全一致
        self.feat_maps = []

        # ✅ [修改点 2B] 修改循环逻辑以正确处理 BasicLayer 和 Downsample
        # 原始的 self.layers 列表是 [BasicLayer1, Downsample1, BasicLayer2, Downsample2, ...]
        # 我们需要成对地处理它们
        num_stages = len(self.embed_dims)
        for i in range(num_stages):
            # 获取当前 Stage 的 BasicLayer 和 Downsample
            basic_layer = self.layers[i * 2]
            downsample_layer = self.layers[i * 2 + 1]

            # 执行 BasicLayer，并接收返回的特征列表和最终输出
            feats_from_layer, x = basic_layer(x)

            # 收集这个 Stage 内部的所有 Block 特征
            # (可以根据 self.feat_ids 进行过滤，这里为简化，全部收集)
            self.feat_maps += feats_from_layer
